...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_embed = self.embedding(captions)  # (N, T, W)
        caption_embed = self.positional_encoding(caption_embed)  # (N, T, W)
        
        # Project image features to same dimension
        visual_embed = self.visual_projection(features).unsqueeze(1)  # (N, 1, W)
        
        # Create target mask to prevent attending to future tokens
        tgt_mask = torch.tril(torch.ones(T, T)).bool()
        
        # Apply transformer decoder
        decoder_output = self.transformer(caption_embed, visual_embed, tgt_mask=tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # Convert image to patches and add positional encoding
        ...
        encoded = self.transformer(patch_embeddings)  # (N, num_patches, embed_dim)
        
        # Average pool all patch vectors
        pooled = torch.mean(encoded, dim=1)  # (N, embed_dim)
