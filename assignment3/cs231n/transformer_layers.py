...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        # Linear projections
        Q = self.query(query).view(N, S, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, S, E/H)
        K = self.key(key).view(N, T, self.n_head, self.head_dim).transpose(1, 2)      # (N, H, T, E/H)
        V = self.value(value).view(N, T, self.n_head, self.head_dim).transpose(1, 2)  # (N, H, T, E/H)
        
        # Scaled dot-product attention
        scores = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(self.head_dim)  # (N, H, S, T)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = patches.contiguous().view(N, C, -1, self.patch_size, self.patch_size)
        patches = patches.permute(0, 2, 1, 3, 4).contiguous()
        patches = patches.view(N, self.num_patches, -1)
