...
class CaptioningRNN:
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            ...
            # (2) Step forward
            if self.cell_type == "rnn":
                next_h = rnn_step_forward(x_embed, prev_h, Wx, Wh, b)
            elif self.cell_type == "lstm":
                next_h, next_c = lstm_step_forward(x_embed, prev_h, prev_c, Wx, Wh, b)
                prev_c = next_c

            # (3) Vocabulary scores
            scores = next_h @ W_vocab + b_vocab  # (N, V)

            # (4) Pick highest-scoring word
            next_word = torch.argmax(scores, dim=1)

            # Save word
            captions[:, t] = next_word

            # Update hidden + prev_word
            prev_h = next_h
            prev_word = next_word
        ############################################################################
        #                             END OF YOUR CODE                             #
